...
class ChromaClient:
        ...
        try:
            ...
            if metadata is None:
                ...
                for i, meta in enumerate(metadata):
                    # Filter out None values and ensure all values are valid types
                    filtered_meta = {}
                    for key, value in meta.items():
                        if value is not None:
                            # Convert to string if not already a basic type
                    ...
                    # Add required fields
                    filtered_meta.update({
                        "student_id": student_ids[i],
                        "class_code": class_code,
                        "inserted_at": datetime.now().isoformat()
                    })
                    metadata[i] = filtered_meta
    ...
    def list_collections(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for collection in collections:
                # Use collection name directly as class code
                class_code = collection.name
    ...
    def cleanup_old_data(self, days_old: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for collection in collections:
                # Process all collections (no prefix filtering)
